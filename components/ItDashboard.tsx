...
import {
    ...
    AppNotification, LeaveStatus
} from '../types';
import {
    Home, Users, Building, Clock, CalendarOff, Database, LogOut, FileDown, CalendarDays, Trash, ShieldCheck, RoleDisplayBadge, MapPin, MoreVertical
} from './icons';
...
type Tab = 'dashboard' | 'employees' | 'departments' | 'logs' | 'leaves' | 'export' | 'system' | 'tracking';
...
const ItDashboard: React.FC<ItDashboardProps> = (props) => {
    ...
    const [editingEmployeeTimesheet, setEditingEmployeeTimesheet] = useState<EmployeeInfo | null>(null);
    const [editingTimesheetDate, setEditingTimesheetDate] = useState<string | null>(null);

    const backupInputRef = useRef<HTMLInputElement>(null);

    const handleEditRecord = (record: DailyTimeRecord) => {
        const employee = props.employees.find(e => e.id === record.employeeId);
        if (employee) {
            setEditingEmployeeTimesheet(employee);
            setEditingTimesheetDate(record.date);
        }
    };
            ...
            case 'logs':
                return (
                     <div className="space-y-4">
                        ...
                        <TimeLogTable dailyRecords={props.dailyRecords} leaveRecords={props.leaveRecords} onClearLogs={() => setConfirmClearLogsOpen(true)} department={logDepartmentFilter} isAdmin={true} onDeleteRecord={props.onDeleteRecord} onEditRecord={handleEditRecord} canEditLogs={true}/>
                    </div>
                );
            case 'tracking':
                return <EmployeeMap departments={props.departments} dailyRecords={props.dailyRecords} employees={props.employees}/>;
            case 'leaves':
                return <LeaveManager employees={props.employees} leaveRecords={props.leaveRecords} leaveActions={props.leaveActions} userRole="it" departments={props.departments} />;
            case 'export':
    ...
    const desktopNavTabs = TABS.concat([
        { id: 'departments', name: 'Departments', icon: Building },
        { id: 'leaves', name: 'Leave Requests', icon: CalendarOff },
        { id: 'export', name: 'Data Export', icon: FileDown },
    ]).sort((a, b) => TABS.findIndex(t => t.id === a.id) - TABS.findIndex(t => t.id === b.id));
    ...
    return (
        <div className="md:flex h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
            ...
            />
             {editingEmployeeTimesheet && editingTimesheetDate && (
                <TimesheetEditModal
                    employee={editingEmployeeTimesheet}
                    startDate={editingTimesheetDate}
                    endDate={editingTimesheetDate}
                    dailyRecords={props.dailyRecords}
                    leaveRecords={props.leaveRecords}
                    onSave={(updatedRecords) => {
                        props.onUpdateTimeLogs(updatedRecords);
                        setEditingEmployeeTimesheet(null);
                        setEditingTimesheetDate(null);
                    }}
                    onClose={() => {
                        setEditingEmployeeTimesheet(null);
                        setEditingTimesheetDate(null);
            ...
            <aside className="w-64 bg-white dark:bg-gray-800 p-4 space-y-2 flex-shrink-0 hidden md:flex md:flex-col">
                <h1 className="text-2xl font-bold text-center text-green-600 dark:text-green-400">IT Panel</h1>
                <nav className="flex flex-col space-y-1 flex-grow">
                    {desktopNavTabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id as Tab)} className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-200' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                ...
                <header className="flex justify-between items-center p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-xl font-semibold capitalize">{TABS.find(t => t.id === activeTab)?.name}</h2>
                     <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <NotificationBell notifications={props.notifications.filter(n => n.recipientId === 'it')} onOpen={() => props.onMarkNotificationsAsRead('it')} />
                        <ThemeToggle />
